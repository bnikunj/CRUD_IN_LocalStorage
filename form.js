      ...
      if (emp_id.value && emp_name.value && emp_gender.value && emp_salary.value) {
        ...
        if (res) {
          if (emp_name.value.length > 2) {
            if (emp_salary.value >= 0 && (emp_gender.value=== 'male' ||emp_gender.value === 'female')) {
              // insert form data into array variable
              dataArray.push(data);
              dataArray = dataArray.concat(JSON.parse(localStorage.getItem('EmployeeData')||'[]'));
          ...
          alert('UserID Invalid, you should fill with Lowercase Letters (a-z), Uppercase Letters (A-Z), Numbers (0-9), Dots (.) and, Underscores (_).');
        }
      }
    }
  } else {
